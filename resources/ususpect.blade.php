...
<div class="container mt-3">
   <center> <h2>Update into Suspect Database</h2></center>
 <br>
<?php
$con = mysqli_connect("127.0.0.1","root","","crimedatabase");
$sr="select * from current_id where type='upsuspect'";
$run_sr = mysqli_query($con,$sr);
$row_s=mysqli_fetch_array($run_sr);
$id=$row_s['c_id'];
$qr="select * from suspect where s_id='$id'";
    ...
    <div class="container-contact100-form-btn">
        ...
        if(isset($_GET['submit'])){
           ...
           /* echo "$c_date * $c_detail * $c_place";*/
            if(strlen($name)>0){
                $insert="UPDATE suspect SET name='$name'where s_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($age)>0){
                $insert="UPDATE suspect SET age='$age'where s_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($job)>0){
                $insert="UPDATE suspect SET job='$job'where s_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($adrs)>0){
                $insert="UPDATE suspect SET address='$adrs'where s_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($phn)>0){
                $insert="UPDATE suspect SET phn='$phn'where s_id='$crid'";
                $query = mysqli_query($con, $insert);}
             $sr="delete from current_id where type='upsuspect'";
            $run_sr = mysqli_query($con,$sr);
