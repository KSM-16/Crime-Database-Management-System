...
<div class="container mt-3">
   <center> <h2>Update into Criminal Database</h2></center>
 <br>
<?php
$con = mysqli_connect("127.0.0.1","root","","crimedatabase");
$sr="select * from current_id where type='upcriminal'";
$run_sr = mysqli_query($con,$sr);
$row_s=mysqli_fetch_array($run_sr);
$id=$row_s['c_id'];
$qr="select * from criminal where c_id='$id'";
...
<form class="contact100-form validate-form">
    ...
    <div class="wrap-input100 rs1-wrap-input100 validate-input" >
        ...
        <input class="input100" type="text" name="job" placeholder=<?php echo $cj;?>>
        <span class="focus-input100"></span>
    </div>
    <div class="wrap-input100 rs1-wrap-input100 validate-input" >
    ...
    <div class="container-contact100-form-btn">
        ...
        if(isset($_GET['submit'])){
            ...
            $job=htmlentities(mysqli_real_escape_string($con,$_GET['job']));
           ...
           /* echo "$c_date * $c_detail * $c_place";*/
            if(strlen($name)>0){
                $insert="UPDATE criminal SET name='$name'where c_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($age)>0){
                $insert="UPDATE criminal SET age='$age'where c_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($job)>0){
                $insert="UPDATE criminal SET job='$job'where c_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($adrs)>0){
                $insert="UPDATE criminal SET address='$adrs'where c_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($phn)>0){
                $insert="UPDATE criminal SET phn='$phn'where c_id='$crid'";
                $query = mysqli_query($con, $insert);}
             $sr="delete from current_id where type='upcriminal'";
            $run_sr = mysqli_query($con,$sr);
