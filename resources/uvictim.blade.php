...
<div class="container mt-3">
   <center> <h2>Update into Victim Database</h2></center>
 <br>
<?php
$con = mysqli_connect("127.0.0.1","root","","crimedatabase");
$sr="select * from current_id where type='upvictim'";
$run_sr = mysqli_query($con,$sr);
$row_s=mysqli_fetch_array($run_sr);
$id=$row_s['c_id'];
$qr="select * from victim where v_id='$id'";
    ...
    <div class="container-contact100-form-btn">
        ...
        if(isset($_GET['submit'])){
           ...
           /* echo "$c_date * $c_detail * $c_place";*/
            if(strlen($name)>0){
                $insert="UPDATE victim SET name='$name'where v_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($age)>0){
                $insert="UPDATE victim SET age='$age'where v_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($job)>0){
                $insert="UPDATE victim SET job='$job'where v_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($adrs)>0){
                $insert="UPDATE victim SET address='$adrs'where v_id='$crid'";
                $query = mysqli_query($con, $insert);}
            if(strlen($phn)>0){
                $insert="UPDATE victim SET phn='$phn'where v_id='$crid'";
                $query = mysqli_query($con, $insert);}
             $sr="delete from current_id where type='upvictim'";
            $run_sr = mysqli_query($con,$sr);
