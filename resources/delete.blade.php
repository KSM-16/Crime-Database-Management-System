...
<div class="container mt-3">
   <center> <h2>Delete from Crime Database</h2></center>
 <br>
</div>
 <div class="container">
  <h2>Crime Data :</h2>
  ...
  <input style='float:right' class='btn btn-secondary' type='submit' name='update' value='delete'>
  <?php if(isset($_GET['update'])){
   if(isset($_GET['radio'])){
   $cid=$_GET['radio'];
   $sr="select * from crime where c_id='$cid'";
   $run_user = mysqli_query($con,$sr);
   $row=mysqli_fetch_array($run_user);
   $id=$row['c_id'];
   $dl="delete from report where id='$id'";
   $run=mysqli_query($con,$dl);
   $dl="delete from suspect where id='$id'";
   $run=mysqli_query($con,$dl);
   $ins="delete from criminal where id='$id'";
   $run=mysqli_query($con,$ins);
   $ins="delete from victim where id='$id'";
   $run=mysqli_query($con,$ins);
   $ins="delete from crime where c_id='$id'";
   $run=mysqli_query($con,$ins);
   echo $run;
   if($run){ 
    echo "<script>window.open('delete.blade.php', '_self')</script>"; }
   }/* else {echo "nnn "; }*/
}
/* else {}*/?>
 </form>
...
<div class="container">
  ...
  <input style='float:right' class='btn btn-secondary' type='submit' name='update1' value='delete'>
  <?php if(isset($_GET['update1'])){
   if(isset($_GET['radio1'])){ 
   $cid=$_GET['radio1'];
   $sr="select id,phn,name from criminal where c_id='$cid'";
   $run_user = mysqli_query($con,$sr);
   $row=mysqli_fetch_array($run_user);
   $phn=$row['phn'];
   $id=$row['id'];
   $name=$row['name'];
   $dl="delete from suspect where name='$name' and id='$id' and phn='$phn'";
   $run=mysqli_query($con,$dl);
   $ins="delete from criminal where c_id='$cid'";
   $run=mysqli_query($con,$ins);
   if($run){
    echo "<script>window.open('delete.blade.php', '_self')</script>"; }
   }/* else {echo "nnn "; }*/
...
<div class="container">
  ...
  <input style='float:right' class='btn btn-secondary' type='submit' name='update2' value='delete'>
  <?php if(isset($_GET['update2'])){
   if(isset($_GET['radio2'])){ 
   $cid=$_GET['radio2'];
   $ins="delete from victim where v_id='$cid'";
   $run=mysqli_query($con,$ins);
   if($run){
    echo "<script>window.open('delete.blade.php', '_self')</script>";}
   }/* else {echo "nnn "; }*/
...
<div class="container">
  ...
  <input style='float:right' class='btn btn-secondary' type='submit' name='update3' value='delete'>
  <?php if(isset($_GET['update3'])){
   if(isset($_GET['radio3'])){ echo $_GET['radio3'];
   $cid=$_GET['radio3'];
   $ins="delete from suspect where s_id='$cid'";
   $run=mysqli_query($con,$ins);
   if($run){
    echo "<script>window.open('delete.blade.php', '_self')</script>";}
   }/* else {echo "nnn "; }*/
...
<div class="container">
  ...
  <input style='float:right' class='btn btn-secondary' type='submit' name='update4' value='delete'>
  <?php if(isset($_GET['update4'])){
   if(isset($_GET['radio4'])){ 
   $cid=$_GET['radio4'];
   $ins="delete from report where r_id='$cid'";
   $run=mysqli_query($con,$ins);
   if($run){
    echo "<script>window.open('delete.blade.php', '_self')</script>";}
   }/* else {echo "nnn "; }*/
